...
checkRole($_SESSION['sess_role'], 'liveconcms_addmenu');	
...
if (isset($_POST['submit']))
{
 ...
 createLogg($lc_UserID, $liveConCMS_logg9);
 ...
 $MenyTitle = "EmptyTitle";
  ...
  foreach($_POST as $key => $val)
  {
	...
	$sql = "INSERT INTO lc_tblmeny (MenyLink, Active, sort, Datum) VALUES('$fileTitle', '1', '$sortNr', '$Dagensdatum')";
	mysql_query_simple_exequter($sql);
				
					$sql = "SELECT ID FROM `lc_tblmeny` WHERE MenyLink = '$fileTitle' AND Datum = '$Dagensdatum'";
			...
			$sql = "INSERT INTO lc_tblheader(ID) VALUES('$newMenyID')";
			mysql_query_simple_exequter($sql);	
			
			$sql = "INSERT INTO lc_tblpages (MenyID, SubID, Datum) VALUES('$newMenyID', '0', '$Dagensdatum')";
			mysql_query_simple_exequter($sql);
			
			$sql = "SELECT ID FROM `lc_tblpages` WHERE MenyID = '$newMenyID' AND Datum = '$Dagensdatum'";
			$result = mysql_query_exequter_with_return($sql);
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{				
				$newpageID = $row["ID"];					
								...
								{				
									...
									$sql = "INSERT INTO lc_tblmenytext(LanguageID, MenyID, MenyText) VALUES('$LanguageID[$i]', '$newMenyID', '$MenyTitle')";
									mysql_query_simple_exequter($sql);
							
									$sql = "INSERT INTO lc_tblheadertext(headerID, LanguageID, headerTitle, headerText, headerLogo) VALUES('$newMenyID', '$LanguageID[$i]', 'Header title here', ' Header text here', '')";
									mysql_query_simple_exequter($sql);
			   ...
			   $_SESSION['sess_tmpnoticehuvudmeny'] = 1;
	...
	$Dagensdatum = date('Y-m-d H:i:s');
	$sql = "INSERT INTO lc_tblmeny (MenyLink, Active, sort, Datum) VALUES('$fileTitle.php', '1', '$sortNr', '$Dagensdatum')";
	mysql_query_simple_exequter($sql);
				
					$sql = "SELECT ID FROM `lc_tblmeny` WHERE MenyLink = '$fileTitle.php' AND Datum = '$Dagensdatum'";
					$result = mysql_query_exequter_with_return($sql);
					while ($row = mysql_fetch_array($result, MYSQL_ASSOC))
					{				
						$newMenyID = $row["ID"];					
	...
	$(document).ready(function(){
		...
		$("#lc-menu-form input:submit").click(function(e){
			...
			if ( type === "type=2" ) 
			{
				if ( file.length === 0 ) 
					{ 
					lcErrorPopup('addsubmenu-2'); return false; 
					}
			}			
...
<div class="bodywrapper container_12">
			...
			<h1 class="page-title"><?PHP echo "$liveConCMS_PageTitle_2"; ?></h1>
			<p><?PHP echo "$liveConCMS_pageText2_1"; ?></p>
				
			<hr />
		
			<div class="ui-widget ui-widget-content ui-corner-all" id="lc-template-form">
				<span class="ui-widget-header ui-corner-top ui-helper-clearfix"><h3><?PHP echo "$liveConCMS_WindowTitle_2"; ?></h3></span>
					<div class="ui-widget-content">
						...
						<fieldset id="template-list">
							<?PHP
							if($manualupload != 1 AND $templateCount != 0)
							{
